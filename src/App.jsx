...
// ═══════════════════════════════════════════════════════════════════════════════
// 🏆 ULTIMATE ENTERPRISE PAYSLIP GENERATOR v3
// Best-of-the-best design with all premium features
// ═══════════════════════════════════════════════════════════════════════════════
...
const LANGUAGES = {
  ...
  hi: {
    ...
    netPay: "शुद्ध वेतन",
    takeHome: "हाथ में आने वाली राशि",
...
async function generatePayslipPDF(emp, data, monthNames, month, year, companyLogo = null, language = 'en') {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  const colors = {
    primary: [25, 103, 210],      // Deep blue
    secondary: [52, 168, 83],     // Green
    accent: [242, 113, 28],       // Orange
    dark: [31, 41, 55],           // Dark gray
    light: [243, 244, 246],       // Light gray
    danger: [220, 38, 38],        // Red
    text: [55, 65, 81],           // Text color
    border: [229, 231, 235],      // Border
  };
  
  const fmt = (val) => `Rs. ${Number(val || 0).toLocaleString("en-IN")}`;
  let y = mY;
  ...
  doc.line(mX, 3, W - mX, 3);
  doc.line(mX, 41, W - mX, 41);
  
  // Company Logo
  if (companyLogo) {
    try {
      doc.addImage(companyLogo, "PNG", mX, 6, 10, 10);
    } catch (err) {}
  }
  ...
  doc.setFontSize(16);
  doc.text("HRPulse", mX + (companyLogo ? 12 : 0), 12);
  
  doc.setTextColor(...colors.text);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(7);
  doc.text("Enterprise HR Management", mX + (companyLogo ? 12 : 0), 17);
  
  // Payslip Title (Right)
  doc.setTextColor(...colors.primary);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(14);
  doc.text(lang.payslip, W - mX, 10, { align: "right" });
  
  doc.setTextColor(...colors.text);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.text(`${monthNames[month - 1]} ${year}`, W - mX, 16, { align: "right" });
  doc.text(`ID: ${emp.code || "EMP-001"}`, W - mX, 20, { align: "right" });
  
  y = 44;

  // ═════════════════════════════════════════════════════════════════════════════
  // EMPLOYEE INFORMATION - Clean Card
  // ═════════════════════════════════════════════════════════════════════════════
  ...
  doc.roundedRect(mX, y, cW, 28, 2, 2, "S");
  
  doc.setTextColor(...colors.primary);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(12);
  doc.text(emp.name, mX + 3, y + 5);
  
  doc.setTextColor(...colors.text);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.text(`${emp.title || "Employee"} • ${emp.dept || "General"}`, mX + 3, y + 10);
  
  const col1 = mX + 3;
  const col2 = W / 2 - 5;
  
  doc.setFontSize(7);
  doc.text(`ID: ${emp.code || "—"}`, col1, y + 16);
  doc.text(`Designation: ${emp.title || "—"}`, col1, y + 21);
  doc.text(`Dept: ${emp.dept || "—"}`, col1, y + 26);
  
  doc.text(`Email: ${emp.email || "—"}`, col2, y + 16);
  const bankField = emp.bankName || emp.bank_name || emp.bankAccount || emp.bank_account || "—";
  doc.text(`Bank: ${bankField}`, col2, y + 21);
  doc.text(`Date: ${new Date().toLocaleDateString("en-IN")}`, col2, y + 26);
  
  y += 32;

  // ═════════════════════════════════════════════════════════════════════════════
  // ATTENDANCE - Horizontal Layout
  // ═════════════════════════════════════════════════════════════════════════════
  
  doc.setTextColor(...colors.text);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(9);
  doc.text(lang.attendance, mX, y);
  y += 5;
  
  const attBoxW = (cW - 6) / 4;
  const attMetrics = [
    { label: "Working Days", value: data.workingDays, color: colors.primary },
    { label: "Present", value: data.present, color: colors.secondary },
    { label: "Absent", value: data.absent, color: colors.danger },
    { label: "Late", value: data.late, color: colors.accent },
  ];
  
  let attX = mX;
  attMetrics.forEach(m => {
    doc.setDrawColor(...m.color);
    doc.setLineWidth(1);
    doc.rect(attX, y, attBoxW, 16, "S");
    
    doc.setTextColor(...m.color);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(12);
    doc.text(String(m.value), attX + attBoxW / 2, y + 8, { align: "center" });
    
    doc.setTextColor(...colors.text);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(6.5);
    doc.text(m.label, attX + attBoxW / 2, y + 14, { align: "center" });
  ...
  doc.rect(mX, y, cW / 2 - 1, 5, "F");
  doc.setTextColor(255, 255, 255);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8);
  doc.text(lang.earnings, mX + 2, y + 3.5);
  ...
  earningItems.forEach((item, idx) => {
    const bgColor = idx % 2 === 0 ? colors.light : [255, 255, 255];
    doc.setFillColor(...bgColor);
    doc.rect(mX, y, cW / 2 - 1, 4, "F");
    
    doc.setTextColor(...colors.text);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7);
    doc.text(item[0], mX + 2, y + 2.5);
    
    doc.setTextColor(...colors.secondary);
    doc.setFont("helvetica", "bold");
    doc.text(fmt(item[1]), mX + cW / 2 - 3, y + 2.5, { align: "right" });
  ...
  doc.rect(W / 2 + 1, y - 26, cW / 2 - 1, 5, "F");
  doc.setTextColor(255, 255, 255);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8);
  doc.text(lang.deductions, W / 2 + 3, y - 22.5);
  ...
  dedItems.forEach((item, idx) => {
    const bgColor = idx % 2 === 0 ? colors.light : [255, 255, 255];
    doc.setFillColor(...bgColor);
    doc.rect(W / 2 + 1, dedY, cW / 2 - 1, 4, "F");
    
    doc.setTextColor(...colors.text);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7);
    doc.text(item[0], W / 2 + 3, dedY + 2.5);
    
    doc.setTextColor(...colors.danger);
    doc.setFont("helvetica", "bold");
    doc.text(fmt(item[1]), W - mX - 2, dedY + 2.5, { align: "right" });
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8);
  doc.text(lang.grossEarnings, mX + 2, y + 3.5);
  doc.setTextColor(...colors.primary);
  doc.text(fmt(data.gross), W - mX - 2, y + 3.5, { align: "right" });
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8);
  doc.text(lang.totalDeductions, mX + 2, y + 3.5);
  doc.setTextColor(...colors.danger);
  doc.text(fmt(data.deductions), W - mX - 2, y + 3.5, { align: "right" });
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(9);
  doc.text(lang.netPay, mX + 3, y + 4);
  
  doc.setFontSize(16);
  doc.text(fmt(data.net), W - mX - 3, y + 10, { align: "right" });
  
  doc.setFontSize(7);
  doc.text(lang.takeHome, mX + 3, y + 14);
  ...
  doc.rect(mX, y, 30, 30, "F");
  doc.setTextColor(...colors.text);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(6);
  doc.text(lang.verificationCode, mX + 15, y + 14, { align: "center" });
  doc.text("[QR Code]", mX + 15, y + 18, { align: "center" });
  
  // Signature Area
  doc.setDrawColor(...colors.border);
  doc.setLineWidth(0.3);
  doc.line(mX + 35, y + 26, mX + 65, y + 26);
  doc.setTextColor(...colors.text);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(6);
  doc.text(lang.signature, mX + 50, y + 28, { align: "center" });
  
  // Company Seal
  doc.setDrawColor(...colors.primary);
  doc.setLineWidth(0.5);
  doc.circle(W - mX - 15, y + 15, 12, "S");
  doc.setTextColor(...colors.primary);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(7);
  doc.text("OFFICIAL", W - mX - 15, y + 14, { align: "center" });
  doc.text("SEAL", W - mX - 15, y + 17, { align: "center" });
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(6);
  doc.text("This is a confidential, system-generated payslip and is valid without physical signature.", W / 2, footerY + 2, { align: "center" });
  doc.text("For discrepancies, contact HR at hr@company.com | +91-XXX-XXX-XXXX", W / 2, footerY + 5, { align: "center" });
  
  doc.setTextColor(150, 160, 170);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(5);
  doc.text("HRPulse • Enterprise HR Solution • hrpulse.io", mX, footerY + 10);
  doc.text(`Generated: ${new Date().toLocaleString("en-IN")} | Page 1 of 1`, W - mX, footerY + 10, { align: "right" });

  return doc;
}

// ═════════════════════════════════════════════════════════════════════════════
// HANDLER WITH EMAIL DELIVERY
// ═════════════════════════════════════════════════════════════════════════════

const handleDownloadPayslip = async (sendEmail = false) => {
  if (!selData) {
  ...
  try {
    ...
    const bs = parseFloat(emp.basic_salary) || 0;
    ...
    const doc = await generatePayslipPDF(emp, payrollData, monthNames, month, year, companyLogo, 'en');
...
// ─── PAYROLL PAGE ─────────────────────────────────────────────────────────────
function PayrollPage({ allEmps, allAtt, isAdmin, setAllEmps, useDemo }) {
  const now = new Date();
  ...
  // Load bank profile when employee is selected
  useEffect(() => {
    if (!sel || empProfiles[sel]) return;
  ...
  const handleSendEmail = async () => {
    if (!selData) return;
    const email = selData.emp.email;
    if (!email) { toast.error("No email address on file for this employee"); return; }

    const confirmed = window.confirm(
      `Send payslip to ${selData.emp.name}?\nEmail: ${email}`
    );
    if (!confirmed) return;

    setMailLoading(true);
    try {
      toast.loading("Sending payslip…", { id: "mail" });

      if (!useDemo) {
        // Generate PDF and send as base64 to backend
        ...
        await api.post("/payroll/send-payslip", {
          ...
          net_pay:     selData.net,
          gross_pay:   selData.gross,
          deductions:  selData.deductions,
...
<button className="btn btn-p" onClick={() => {
          ...
          a.href     = url;
          a.download = `payroll_${monthNames[month - 1]}_${year}.csv`;
        ...
        <button className="btn btn-g" onClick={() => {
          ...
          if (!configured.length) { toast.error("No employees have salary configured yet."); return; }
          const ok = window.confirm(
            `Send payslips to ${configured.length} employee(s)?\n\n` +
            ...
            {payrolls.map(({ emp, net, gross, deductions, present, absent, late }) => (
              <div
                ...
                style={{
                  padding: "12px 20px",
                  cursor: "pointer",
                  background:   sel === emp.id ? "var(--gd)" : "",
                  borderLeft:   sel === emp.id ? "3px solid var(--g)" : "3px solid transparent",
                }}
                onClick={() => { setSel(sel === emp.id ? null : emp.id); setEditingSalary(false); setShowAccNum(false); }}
              >
                <Av emp={emp} size={36}/>
                <div style={{ flex:1, minWidth:0 }}>
                ...
                {!editingSalary ? (
                  ...
                  <div>
                    ...
                    {salaryForm.base_salary > 0 && (() => {
                      const b  = +salaryForm.base_salary;
                      const g  = b + Math.round(b * (+salaryForm.hra_pct  || 0) / 100) + (+salaryForm.ta_amount || 0);
                      const n  = Math.max(0, g - Math.round(b * (+salaryForm.pf_pct || 0) / 100) - Math.round(b * (+salaryForm.tax_pct || 0) / 100));
                      return (
                        <div style={{ background:"var(--s2)", borderRadius:8, padding:"10px 12px", marginTop:4 }}>
                          <div style={{ color:"var(--text3)", marginBottom:4, fontSize:10, letterSpacing:.5 }}>PREVIEW</div>
                          <div style={{ display:"flex", justifyContent:"space-between" }}>
                            <span style={{ color:"var(--text2)", fontSize:12 }}>Gross</span>
                            <span style={{ color:"var(--g)", fontWeight:700 }}>{rupee(g)}</span>
                          </div>
                          <div style={{ display:"flex", justifyContent:"space-between", marginTop:3 }}>
                            <span style={{ color:"var(--text2)", fontSize:12 }}>Est. Net</span>
                            <span style={{ color:"var(--g)", fontWeight:700 }}>{rupee(n)}</span>
                          </div>
                        </div>
                      );
                    })()}
            ...
            <div className="payroll-card">
              ...
              <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16 }}>
                ...
                <div style={{ marginLeft:"auto", display:"flex", gap:8 }}>
                  <button
                    className="btn btn-g"
                    style={{ fontSize:11, opacity: pdfLoading ? .6 : 1 }}
                    disabled={pdfLoading || selData.baseSalary === 0}
                    onClick={handleDownloadPDF}
                    title={selData.baseSalary === 0 ? "Set salary first" : "Download PDF payslip"}
                  >
                    {pdfLoading ? <Spin/> : "📥 Download PDF"}
                  </button>
                  <button
                    className="btn btn-s"
                    style={{ fontSize:11, opacity: mailLoading ? .6 : 1 }}
                    disabled={mailLoading || selData.baseSalary === 0}
                    onClick={handleSendEmail}
                    title={selData.baseSalary === 0 ? "Set salary first" : `Send payslip to ${selData.emp.email}`}
                  >
                    {mailLoading ? <Spin/> : "📧 Send Email"}
                  </button>
                </div>
              </div>
              ...
              {/* Pay period label */}
              <div style={{ fontSize:10, color:"var(--text3)", letterSpacing:1, marginBottom:10 }}>
                PAY PERIOD · {monthNames[month - 1].toUpperCase()} {year}
              </div>
              ...
              {selData.baseSalary === 0 ? (
                ...
                <>
                  ...
                  {[["PF", selData.pf], ["Tax", selData.tax], ["Absent", selData.absentDeduction], ["Late", selData.lateDeduction]]
                    .filter(([, v]) => v > 0)
                    .map(([l, v]) => (
                      <div key={l} style={{ display:"flex", justifyContent:"space-between", padding:"5px 0", borderBottom:"1px solid rgba(255,255,255,0.04)", fontSize:12 }}>
                        <span style={{ color:"var(--text2)" }}>{l}</span>
  ...
  if (user && isCompanySuspended) return (
    <div style={{ minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16,background:"var(--bg)",textAlign:"center",padding:24 }}>
      ...
      <div style={{ marginTop:8,padding:"12px 20px",background:"rgba(239,68,68,0.06)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:12,fontSize:13,color:"var(--text2)" }}>
        📧 <a href="" style={{color:"#3B82F6",textDecoration:"none"}}>support@hrpulse.io</a>
        &nbsp;·&nbsp; 📞 +91 8800935457
      </div>
  ...
  return (
    <div style={{ minHeight:"100vh",display:"flex",background:"var(--bg)" }}>
      ...
      <main style={{ flex:1,overflowY:"auto",padding:"26px 28px" }}>
        ...
        {nav==="Payroll" && <PayrollPage allEmps={allEmps} allAtt={allAtt} isAdmin={isAdmin} setAllEmps={setAllEmps} useDemo={useDemo}/>}
