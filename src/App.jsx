...
function EarlyCheckoutModal({ workedMinutes, onConfirm, onCancel }) {
  const [reason, setReason] = useState("");
  const hrs = Math.floor(workedMinutes / 60);
  const mins = workedMinutes % 60;
  return (
    <div>
      <div style={{ fontSize:18, fontWeight:800, color:"var(--text)", marginBottom:16, letterSpacing:-.3 }}>
        ⚠️ Early Checkout
      </div>
      <div style={{ background:"rgba(245,158,11,0.08)", border:"1px solid rgba(245,158,11,0.25)", borderRadius:12, padding:"14px 16px", marginBottom:18 }}>
        <div style={{ fontSize:13, color:"#F59E0B", fontWeight:600, marginBottom:4 }}>
          You've only worked {hrs}h {mins}m today
        </div>
        <div style={{ fontSize:12, color:"var(--text3)" }}>
          Standard hours are 8h. Please provide a reason for leaving early.
        </div>
      </div>
      <div style={{ marginBottom:8 }}>
        <label style={{ fontSize:10, color:"var(--text3)", fontWeight:700, letterSpacing:1, textTransform:"uppercase", display:"block", marginBottom:5 }}>
          Reason for early checkout
        </label>
        <textarea rows={3} value={reason} onChange={e=>setReason(e.target.value)} placeholder="e.g. Doctor appointment, family emergency..." style={{ resize:"vertical", width:"100%" }}/>
      </div>
      <div style={{ display:"flex", gap:10, justifyContent:"flex-end", marginTop:18 }}>
        <button className="btn btn-g" onClick={onCancel}>Cancel</button>
        <button className="btn btn-w" onClick={()=>onConfirm(reason)} disabled={!reason.trim()}>
...
export default function App() {
  ...
  const checkOut=async(selfie=null, earlyReason=null)=>{
    ...
    try {
      if (!useDemo) {
        ...
        const d=await api.post("/attendance/checkout",{latitude:loc.lat,longitude:loc.lng,selfie_base64:selfie,...(earlyReason?{early_reason:earlyReason}:{})});
        toast.success(`${d.message}${d.work_minutes?` · ${fmtH(d.work_minutes)} worked`:""}`); loadAll();
      } else {
        await new Promise(r=>setTimeout(r,600));
        const now=new Date();
        const workedMins = todayRec?.check_in ? Math.round((now-new Date(todayRec.check_in))/60000) : 0;
        const update = r => ({...r, check_out:now.toISOString(), work_minutes:Math.round((now-new Date(r.check_in))/60000), ...(earlyReason?{early_checkout:true,early_reason:earlyReason}:{})});
        setAtt(p=>p.map(r=>r.date===todayStr()?update(r):r));
        setAllAtt(p=>p.map(r=>r.date===todayStr()&&r.employee_id===user.id?update(r):r));
        if (earlyReason) {
          toast.success(`Early checkout recorded (${fmtH(workedMins)} worked) ⚠️`);
        } else {
  ...
  const handleCheckOut = ()=>{
    // Check for early checkout (< 8 hours / 480 min worked)
    if (todayRec?.check_in) {
      const workedMinutes = Math.round((Date.now() - new Date(todayRec.check_in).getTime()) / 60000);
      if (workedMinutes < 480) {
        setModal({type:"earlyCheckout", workedMinutes});
        return;
      }
    }
    setModal({type:"selfie",action:"out"});
  };
  ...
  return (
    <div style={{ minHeight:"100vh",display:"flex",background:"var(--bg)" }}>
      <style>{CSS}</style>
      <Toaster position="top-right" toastOptions={{ style:{ background:"#0d1528",color:"#E8EDF5",border:"1px solid rgba(255,255,255,0.11)",fontSize:13,borderRadius:12,boxShadow:"0 8px 32px rgba(0,0,0,0.5)" } }}/>
      ...
      {modal&&(
        <div className="modal-bg" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
          <div className="modal">
            {modal.type==="selfie"
              ? <SelfieCapture onCapture={photo=>{setModal(null);modal.action==="in"?checkIn(photo):checkOut(photo,modal.earlyReason||null);}} onSkip={()=>{setModal(null);modal.action==="in"?checkIn(null):checkOut(null,modal.earlyReason||null);}}/>
              : modal.type==="earlyCheckout"
              ? <EarlyCheckoutModal workedMinutes={modal.workedMinutes} onConfirm={(reason)=>{setModal({type:"selfie",action:"out",earlyReason:reason});}} onCancel={()=>setModal(null)}/>
